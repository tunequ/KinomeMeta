...
task_range = 'all661'
_,short_name_dict = get_task_info('all661')     #get kinase name mapping dict for 661 kinases in our article
all_task_list =[task for task in os.listdir(args.baselearner_dir) if '.pth' in task]


#save_path
if args.save_path is None:
    args.save_path = f'./results/prediction/prediction_{tasktime}_{os.path.basename(path)}'
...
for i, task in enumerate(tqdm(all_task_list)):
    try:
        model_path = os.path.join(args.baselearner_dir,task)
...
def assign_label(pred_df, dataset_smiles_all_wash, short_name_dict, pos_smi_dict_pickle_path,pred_position):
    pred_df_1 = assign_bc_hit_label(pred_df, dataset_smiles_all_wash,pred_position)
    print('assign bc hit and label success!')
    pred_df_2 = pred_df_1.rename(columns=short_name_dict)
    print('rename success!')
    pred_df_3 = assign_task_smi(pos_smi_dict_pickle_path, pred_df_2)
...
final_pre_result.to_csv(save_path, index=False)
