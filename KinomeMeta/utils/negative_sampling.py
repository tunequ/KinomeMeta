...
def mol2decoys(mol, support_fps, decoys_dict, sim_cutoff=0.6, radio = 3):
    ...
    for decoy in decoys:
        decoys_sims = DataStructs.BulkTanimotoSimilarity(
            AllChem.GetMorganFingerprintAsBitVect(decoy, 3, 1024),
            support_fps)
...
def generate_neg_samples(data_path,save_path):
    ...
    decoys_df = pd.DataFrame(columns=data_df.columns[1:], index=set(sum(list(smiles_decoys_dict.values()), [])))
