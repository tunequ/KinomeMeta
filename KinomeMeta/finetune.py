...
if not os.path.exists(path_decoys):
    os.makedirs(os.path.dirname(path_decoys),exist_ok=True)
    generate_neg_samples(path_result,path_decoys)
...
task_df_info, task_df_all, feature_dicts_all, _ = get_smiles(path_result)
...
# with open(os.path.join(save_dir,'result.csv'), 'a') as f:
#     for data in result3:
#         f.write(','.join([str(r) for r in data]))
#         f.write('\n')


result4 = [np.mean(best_mccs), np.mean(best_f1s), np.mean(best_accs), np.mean(best_accs), np.mean(best_pres), np.mean(best_recalls), np.mean(best_rocs), np.mean(best_baccs)]
with open(os.path.join(save_dir,'finetune_results_mean.txt'), 'a') as f:
    f.write(path_decoys)
    f.write('\n')
    f.write('\n'.join([str(r) for r in result4]))
    f.write('\n')
